 ...
 <div class="container-fluid pm-certificate-container">
     ...
     <div class="border border-white p-3 pm-certificate-border text-center">
         ...
         <h3 class="fw-bold fullName my-3 display-6">{{ $user->participant->full_name }}</h3>
         <div class="row justify-content-center">
             <p class="sans col-10 lh-lg">
                 ...
                 <br>
                 Selama
                 {{ Carbon\Carbon::parse($user->participant->start_date)->diffInMonths($user->participant->end_date) }}
                 Bulan
                 terhitung
                 mulai tanggal
                 {{ Carbon\Carbon::parse($user->participant->start_date)->format("d F Y") . " - " . Carbon\Carbon::parse($user->participant->end_date)->format("d F Y") }}
                 <br />
                 Dengan hasil : <strong>
                     {{ getAssessmentCategory($user->participant->assessments->avg("score")) }}
 ...
 <div class="container-fluid pm-certificate-container">
     <div class="row mb-5 align-items-center">
         <div class="col text-start">
             <div class="info-row">
                 ...
                 <span class="info-value">: {{ $user->participant->full_name }}</span>
             </div>
             <div class="info-row">
                 <span class="info-label">Institusi</span>
                 <span class="info-value">: {{ $user->participant->institution }}</span>
             </div>
             <div class="info-row">
                 <span class="info-label">Pelaksanaan Magang</span>
                 <span class="info-value">: {{ Carbon\Carbon::parse($user->participant->start_date)->format("d F Y") }}
                     s.d
                     {{ Carbon\Carbon::parse($user->participant->end_date)->format("d F Y") }}</span>
             </div>
         </div>
         <div class="col-4">
     ...
     <table class="table table-bordered table-sm text-center">
         ...
         <tbody>
             @foreach ($user->participant->assessments as $assessment)
                 <tr>
